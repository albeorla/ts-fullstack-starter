...
setup("authenticate", async ({ page, context }) => {
  ...
  if (!dbReady) {
    throw new Error("Database is not available. Please ensure the database is running before running tests.");
  }
  ...
  try {
    ...
    const loggedInText = await page.locator("text=/Logged in as/i");
    const isVisible = await loggedInText.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    if (!isVisible) {
      ...
      throw new Error("Failed to verify authentication - 'Logged in as' text not found");
    }
